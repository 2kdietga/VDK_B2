...
function render() {
  ...
  modeButtons.forEach((button) => {
    button.classList.toggle(
      "active",
      button.dataset.mode === state.controlMode,
    );
  });
  ...
  controlCards.forEach((card) => {
    card.classList.toggle(
      "active",
      card.dataset.control === state.activeControl,
    );
  ...
  menuItems.forEach((item) => {
    item.classList.toggle(
      "active",
      Number(item.dataset.menuIndex) === state.menuIndex,
    );
  });
}
...
function applyContinuousGesture() {
  ...
  const step = state.gesture === "phai" ? 5 : -5;
...
function updateControlValue(control, value) {
  ...
  if (state[control] === nextValue && state.activeControl === control) {
    return;
  }
...
async function pollState() {
  try {
    ...
    if (
      data.updated_at !== state.lastServerUpdate ||
      data.esp32_online !== state.esp32Online
    ) {
...
for (const control of ["led", "motor"]) {
  ranges[control].addEventListener("input", (event) => {
    ...
    updateControlValue(control, event.target.value);
  });

  ranges[control].addEventListener("change", (event) => {
    if (state.controlMode !== "manual" || state.menuOpen) {
      return;
    }
...
setInterval(applyContinuousGesture, 150);
