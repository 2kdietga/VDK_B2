...
const pauseButtons = Array.from(
  document.querySelectorAll("[data-menu-action]"),
);
...
function updateHUD() {
  ...
  hpFill.style.background =
    pct > 50 ? "#3fb950" : pct > 25 ? "#f0c040" : "#f85149";
...
function draw() {
  ...
  if (!waveActive && betweenWave > 0) {
    ...
    cx.fillText(
      `Tiếp theo sau ${Math.ceil(betweenWave / 60)} giây...`,
      W / 2,
      H / 2 + 16,
    );
    cx.fillStyle = "#21262d";
  ...
  if (gameOver) {
    ...
    cx.fillText(
      `Sóng ${wave} · ${score} điểm  |  Nhấn R / OK để chơi lại`,
      W / 2,
      H / 2 + 18,
    );
  }
  ...
  try {
    ...
    if (data.screen !== "game") {
      ...
      return;
        ...
        poll.btnOk = Boolean(data.btn_ok);
    ...
    document.querySelector("#info").textContent =
      "Mất kết nối server · vẫn có thể chơi bằng bàn phím";
  }
}
...
const attackButton = document.querySelector("#bA");
...
const releaseAttackButton = () => {
    sendInput({ btn_ok: false });
};

attackButton.addEventListener("pointerup", releaseAttackButton);
attackButton.addEventListener("pointerleave", releaseAttackButton);
